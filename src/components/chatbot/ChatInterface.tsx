...
export default function ChatInterface() {
    ...
    setMessages([
      {
        id: 'initial-greeting',
        role: 'assistant',
        content: "Namaste! I am AyurAid, your personal Ayurvedic and alternative medicine guide. How can I help you today?",
        timestamp: new Date(),
      }
    ]);
  }, []);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const aiResponse = await getAyurvedicGuidance({ question: input });
  ...
  return (
    <div className="flex flex-col h-full bg-card rounded-lg shadow-xl overflow-hidden">
        ...
        <div className="space-y-6">
            ...
            >
              ...
              >
                <CardContent className="p-0 text-sm sm:text-base leading-relaxed">
                  <p>{message.content}</p>
                  <p className={`text-xs mt-2 ${message.role === 'user' ? 'text-accent-foreground/70' : 'text-primary-foreground/70'}`}>
          ...
          {isLoading && (
            <div className="flex items-end gap-3 justify-start">
              ...
              <Card className="max-w-xs sm:max-w-md p-3 sm:p-4 rounded-2xl shadow-md bg-primary text-primary-foreground rounded-bl-none">
      ...
      <form
        ...
        className="p-4 sm:p-6 border-t bg-background/50 flex items-center gap-2 sm:gap-4"
      >
        <Input
          value={input}
          onChange={(e) => setInput(e.target.value)}
          placeholder="Ask AyurAid about Ayurveda or alternative medicine..."
          className="flex-grow text-base rounded-full px-4 py-3 focus-visible:ring-primary"
          disabled={isLoading}
        />
        <Button
          type="submit"
